...
class MixturePathGeneralizedKL(_Loss):
    ...
    def forward(self, logits: Tensor, x_1: Tensor, x_t: Tensor, timesteps: Tensor, 
        ...
        # Get scheduler output using timestep indices
...
def test_loss_functions():
    """Test function to verify the loss functions work correctly."""
    ...
    noised_input_ids = torch.randint(0, vocab_size, (batch_size, seq_len))
    ...
    try:
        ...
        print(f"✅ Flow Matching loss works! Loss value: {flow_loss.item():.4f}")
    except Exception as e:
        print(f"❌ Flow Matching loss failed: {e}")
    
    # Test Discrete loss
    try:
        discrete_loss_fn = DiscreteDiffusionLoss(mask_token_id=999, reduction="mean")
        target = input_ids.clone()
        target[noised_input_ids != 999] = -100
        target[attention_mask == 0] = -100
        
        discrete_loss = discrete_loss_fn(
            logits=logits,
            target=target,
            attention_mask=attention_mask
        )
        print(f"✅ Discrete loss works! Loss value: {discrete_loss.item():.4f}")
    except Exception as e:
        print(f"❌ Discrete loss failed: {e}")
