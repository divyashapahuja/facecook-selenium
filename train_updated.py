...
from loss import MixturePathGeneralizedKL, MixtureDiscreteProbPath, create_flow_matching_schedulers
...
def evaluate_model(model, val_loader, loss_fn, accelerator, max_eval_batches=50):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(val_loader):
            ...
            timesteps = torch.randint(0, len(loss_fn.alpha_t), (input_ids.shape[0],), device=accelerator.device)
            time_t = loss_fn.alpha_t[timesteps]  # Use alpha_t as time embedding
            
            noised_input_ids = model._create_noise_ids(input_ids, time_t.unsqueeze(1))
...
def main(
    ...
    # Initialize wandb
    if run_name is None:
        run_name = f"dflm_bs{batch_size}_lr{learning_rate}_wd{weight_decay}_{scheduler_type}"
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            # Prepare targets - only predict tokens that were masked
            target = input_ids.clone()
            target[noised_input_ids != model.mask_token_id] = -100
            target[attention_mask == 0] = -100  
            ...
            if accelerator.is_main_process and step % 100 == 0:  # Reduce print frequency
                ...
                print(f"timesteps range: {timesteps.min().item()}-{timesteps.max().item()}")
                
            # Compute loss using the generalized flow matching loss
            loss = loss_fn(
                input=logits.transpose(-1, -2),  # [B, V, L] -> [B, L, V] for CE loss
                target=target,
            )
...
if __name__ == "__main__":
    ...
    main(
        ...
        max_grad_norm=1.0,
        project_name="discrete-flow-llm",
        run_name="dflm_generalized_flow_matching_v1",
        scheduler_type="linear"  # Try "cosine" or "exponential" for different schedules
    )